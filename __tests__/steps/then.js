...
const user_exist_in_UsersTable = async (id) => {
...
const tweet_exists_in_TweetsTable = async (id) => {
    const DynamoDB = new AWS.DynamoDB.DocumentClient()
    console.log(`looking for tweet in [${id}] in table [${process.env.TWEETS_TABLE}]`)
    const resp = await DynamoDB.get({
...
const tweet_exists_in_TimelinesTable = async (userId, tweetId) => {
    const DynamoDB = new AWS.DynamoDB.DocumentClient()
    console.log(`looking for tweet in [${tweetId}] in table [${process.env.TIMELINES_TABLE}]`)
    const resp = await DynamoDB.get({
